...
struct ReviewSectionView: View {
    ...
    private var audioButton: some View {
        Button(action: { Haptics.softTap(); playAudio() }) {
        ...
        VStack(spacing: 12) {
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(DuoChaosCopy.reviewCaughtUp(count: totalDue).title)
                        .font(themeStore.bold(16))
                        .foregroundStyle(themeStore.mainText)
                    Text(DuoChaosCopy.reviewCaughtUp(count: totalDue).subtitle)
    ...
    private typealias ReviewQuality = SRSScheduler.ReviewGrade
    ...
    private func scheduleNext(for card: WordCard, quality: ReviewQuality) {
        ...
        if let after = result.reinsertAfterCards {
            reinsert(card, after: after)
        }
    ...
    private func computeNextInterval(for card: WordCard, quality: ReviewQuality) -> Int {
        ...
        return SRSScheduler.previewReviewIntervalDays(
            state: SchedulingState(
                easeFactor: w.easeFactor,
                intervalDays: w.intervalDays,
                repetitions: w.repetitions,
    ...
    private func highlightedExample(example: String, target: String) -> AttributedString {
        ...
        if startOK && endOK,
           ...
           let attrEnd = AttributedString.Index(range.upperBound, within: attr) {
            ...
            attr[attrStart..<attrEnd].font = UIFont(name: "Poppins-Bold", size: 15)
